  ...
  try {
    switch (action) {
      ...
      case 'create': {
        ...
        // Use Cognito's built-in temporary password generation
        // It will automatically send the invitation email using our custom template
        await cognito.send(new AdminCreateUserCommand({
          ...
          // Note: temporary password is auto-generated by Cognito 
          // and valid for the duration set in the User Pool (e.g. 12-24 hours)
        }));

        await logAction(adminId, 'CREATE_USER', { name, email, role, department, campus }, event.requestContext?.identity?.sourceIp || '0.0.0.0');
